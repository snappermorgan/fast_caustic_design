...
// Retro 80s-themed status phrases
static const char *retro_phrases[] = {
    "Running up that hill",
    "Rocking the Casbah",
    "Relaxing (Don't do it)",
...
static const int num_retro_phrases = 30;

// Random phrase generator (seeded on first use)
static std::mt19937 &get_rng() {
  static std::mt19937 rng(static_cast<unsigned>(std::time(nullptr)));
  return rng;
}

static int get_random_phrase_index() {
  std::uniform_int_distribution<int> dist(0, num_retro_phrases - 1);
  return dist(get_rng());
}

// Stdout suppression for JSON mode
...
void log_progress(bool json, const std::string &status,
                  const std::string &stage, float progress,
                  const std::string &msg) {
  if (!json)
    return;

  // Temporarily restore stdout to print JSON
  if (original_cout_buffer) {
    std::cout.rdbuf(original_cout_buffer);
  }

  // Get random retro phrase
  const char *phrase = retro_phrases[get_random_phrase_index()];

  // Format: "RetroPhrase (actual status)"
  std::string fun_message = std::string(phrase) + " (" + msg + ")";

  std::cout << "{\"status\": \"" << status << "\", \"stage\": \"" << stage
            << "\", \"progress\": " << progress << ", \"message\": \""
            << fun_message << "\"}" << std::endl;
...
void output_usage() {
  std::cout << "\033[1;36m" << "CAUSTIC DESIGN - 3D Caustic Surface Generator"
            << "\033[0m" << std::endl;
  std::cout << "Generates 3D lens surfaces that create desired light patterns "
               "(caustics)"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  -in_trg <filename>     Target caustic pattern image (what "
               "you want the light to look like)"
            << std::endl;
  std::cout << "                         Supported formats: PNG, BMP, JPG, and "
               "other CImg formats"
            << std::endl;
  std::cout << "                         Must be square (same width and "
               "height) for OT solver"
            << std::endl;
  std::cout << "                         OR use procedural patterns: "
               ":id:resolution: (e.g., :8:256:)"
            << std::endl;
  std::cout << "                         Available patterns: 1=constant, "
               "5=linear, 6=circles+boundary,"
            << std::endl;
  std::cout << "                         8=circles, 10=single black dot, "
               "11=single white dot,"
            << std::endl;
  std::cout << "                         21-25=research examples, "
               "31-33=advanced patterns, etc."
            << std::endl;
  std::cout << std::endl;

  std::cout << "\033[1;33m" << "LENS GEOMETRY OPTIONS:" << "\033[0m"
            << std::endl;
  std::cout << "  -res <value>           Mesh resolution for the caustic "
               "surface (default: 100)"
            << std::endl;
  std::cout << "                         Higher values = more detail but "
               "slower computation"
            << std::endl;
  std::cout << "  -focal_l <value>       Focal length of the caustic lens "
               "(default: 1.0)"
            << std::endl;
  std::cout << "                         Distance from lens to projection plane"
            << std::endl;
  std::cout << "  -thickness <value>     Physical thickness of the lens "
               "(default: 0.2)"
            << std::endl;
  std::cout << "                         Controls how thick the final 3D model "
               "will be"
            << std::endl;
  std::cout
      << "  -mesh_width <value>    Physical width of the lens (default: 1.0)"
      << std::endl;
  std::cout << "                         Height is auto-computed to match "
               "image aspect ratio"
            << std::endl;
  std::cout << std::endl;

  std::cout << "\033[1;33m" << "SOURCE LIGHT OPTIONS:" << "\033[0m"
            << std::endl;
  std::cout
      << "  -in_src <filename>     Source light distribution image (optional)"
      << std::endl;
  std::cout << "                         If not provided, uniform light "
               "distribution is used"
            << std::endl;
  std::cout
      << "                         Same format requirements as target image"
      << std::endl;
  std::cout << std::endl;

  std::cout << "\033[1;33m" << "OUTPUT OPTIONS:" << "\033[0m" << std::endl;
  std::cout
      << "  -output <path>         Output file path (default: './output.obj')"
      << std::endl;
  std::cout << "                         Can specify full path with filename "
               "or just directory"
            << std::endl;
  std::cout << "                         If directory only, 'output.obj' will "
               "be used as filename"
            << std::endl;
  std::cout << std::endl;

  std::cout << "\033[1;33m" << "SOLVER OPTIONS:" << "\033[0m" << std::endl;
  std::cout << "  -beta <method>         Optimization method: 'cj' (conjugate "
               "jacobian) or '0' (zero)"
            << std::endl;
  std::cout << "                         Default: 'cj' (recommended for better "
               "performance)"
            << std::endl;
  std::cout
      << "  -itr <max_iterations>  Maximum solver iterations (default: 1000)"
      << std::endl;
  std::cout << "  -th <threshold>        Convergence threshold (default: 1e-7)"
            << std::endl;
  std::cout << "                         Lower values = more precise but slower"
            << std::endl;
  std::cout
      << "  -ratio <max_ratio>     Maximum target ratio (default: unlimited)"
      << std::endl;
  std::cout << "  -v <level>             Verbosity level 0-10 (default: 1)"
            << std::endl;
  std::cout << "                         0=silent, 1=normal, 10=very detailed"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "    caustic_design -in_trg pattern.png -res 200 -focal_l 2.0"
            << std::endl;
  std::cout << std::endl;
  std::cout << "  With custom source light and output directory:" << std::endl;
  std::cout << "    caustic_design -in_trg target.png -in_src source.png "
               "-output ./results/"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout
      << "  The program generates a 3D model file (.obj format) that can be:"
      << std::endl;
  std::cout << "  * 3D printed to create a physical caustic lens" << std::endl;
  std::cout << "  * Imported into 3D modeling software (Blender, Maya, etc.)"
            << std::endl;
  std::cout << "  * Used in optical simulations" << std::endl;
  std::cout << std::endl;

  std::cout << "\033[1;35m" << "NOTES:" << "\033[0m" << std::endl;
  std::cout << "  * Input images must be square (OT solver limitation)"
            << std::endl;
  std::cout << "  * Higher resolution (-res) gives more detail but takes longer"
            << std::endl;
  std::cout << "  * The lens uses refractive index 1.55 (typical plastic/glass)"
            << std::endl;
  std::cout << "  * Processing involves 10 outer iterations for optimal results"
            << std::endl;
  std::cout << "  * Procedural patterns are useful for testing - try :8:256: "
               "for circles"
            << std::endl;
  std::cout << "  * Output path can be full filename or directory (defaults to "
               "'output.obj')"
            << std::endl;
  std::cout << std::endl;
}
...
struct CLIopts : CLI_OTSolverOptions {
  ...
  bool load(const InputParser &args) {
    ...
    if (args.cmdOptionExists("--json-progress"))
      json_progress = true;
...
template <typename T, typename S> T lerp(S u, const T &a0, const T &a1) {
  return (1. - u) * a0 + u * a1;
}

void interpolate(const std::vector<Surface_mesh> &inv_maps, double alpha,
                 Surface_mesh &result) {
  // clear output
  result.clear();
  result = inv_maps[0];
  ...
  for (int j = 0; j < nv; ++j) {
    ...
    result.position(v) =
        lerp(alpha, inv_maps[0].position(v), inv_maps[1].position(v));
  }
}

void synthetize_and_save_image(const Surface_mesh &map,
                               const std::string &filename, int res,
                               double expected_mean, bool inv) {
...
void gradient(std::vector<double> source, std::vector<double> interf,
              std::vector<double> target, double n1, double n2, double &grad_x,
              double &grad_y) {
  double d1 = std::sqrt((interf[0] - source[0]) * (interf[0] - source[0]) +
                        (interf[1] - source[1]) * (interf[1] - source[1]) +
                        (interf[2] - source[2]) * (interf[2] - source[2]));
  double d2 = std::sqrt((target[0] - interf[0]) * (target[0] - interf[0]) +
                        (target[1] - interf[1]) * (target[1] - interf[1]) +
                        (target[2] - interf[2]) * (target[2] - interf[2]));

  grad_x =
      n1 * (interf[0] - source[0]) / d1 - n2 * (target[0] - interf[0]) / d2;
  grad_y =
      n1 * (interf[1] - source[1]) / d1 - n2 * (target[1] - interf[1]) / d2;
}

void scaleAndTranslatePoints(std::vector<std::vector<double>> &points,
                             double MAX_X, double MAX_Y, double margin) {
...
void export_grid_to_svg(std::vector<std::vector<double>> &points, double width,
                        double height, int res_x, int res_y,
                        std::string filename, double stroke_width) {
  ...
  svg_file << "<svg width=\"1000\" height=\"" << 1000.0f * (height / width)
  ...
  for (int j = 0; j < res_y; j++) {
    ...
    for (int i = 0; i < res_x; i++) {
      ...
      path_str +=
          std::to_string((point[0] / width) * 1000.0f) + "," +
          std::to_string((point[1] / height) * 1000.0f * (height / width));
      if (i < res_x - 1)
        path_str += "L";
    }
    svg_file << "<path d=\"" << path_str
...
void export_triangles_to_svg(std::vector<std::vector<double>> &points,
                             std::vector<std::vector<unsigned int>> &triangles,
                             double width, double height, int res_x, int res_y,
                             std::string filename, double stroke_width) {
  std::ofstream svg_file(filename, std::ios::out);
  if (!svg_file.is_open()) {
    std::cerr << "Error: Unable to open file " << filename << std::endl;
...
void scalePoints(std::vector<std::vector<double>> &trg_pts,
...
void translatePoints(std::vector<std::vector<double>> &trg_pts,
                     std::vector<double> position_xyz) {
...
void rotatePoints(std::vector<std::vector<double>> &trg_pts,
                  std::vector<double> angle_xyz) {
...
TransportMap runOptimalTransport(MatrixXd &density, CLIopts &opts) {
  ...
  std::cout << "STATS solver -- init: " << t_solver_init.value(REAL_TIMER)
            << "s  solve: " << t_solver_compute.value(REAL_TIMER) << "s\n";

  return tmap_src;
}

void applyTransportMapping(TransportMap &tmap_src, TransportMap &tmap_trg,
...
/*std::vector<double> refract(const std::vector<double>& normal, const
std::vector<double>& incident, double n1, double n2) {
    ...
    if (sinT2 > 1.0) {
        ...
        // return Vector::invalid; // Uncomment if you have a way to represent
TIR
    }
...
std::vector<double>
refract(const std::vector<double> &surfaceNormal,
  ...
  double cosThetaI =
      -dotProduct; // Cosine of the angle between the ray and the normal
...
bool intersect_plane(const std::vector<double> &n,
  ...
  if (denom > 1e-6) { // Check if ray is not parallel to the plane
    ...
    if (t >=
        0) { // Check if intersection is in the positive direction of the ray
...
std::vector<double> calc_plane_normal(const std::vector<double> &A,
...
bool is_boundary_vertex(Mesh &mesh,
                        std::vector<std::pair<int, int>> &adjacent_edges,
                        std::vector<int> &adjacent_triangles, int vertex_index,
...
fresnelMapping(std::vector<std::vector<double>> &vertices,
  ...
  // place initial points on the refractive surface where the light rays enter
  // the material
  /*if (use_point_src && !use_reflective_caustics) {
      for(int i = 0; i < vertices.size(); i++) {
          ...
          double t = ((boundary_z - pointLightPosition[2]) / (vertices[i][2] -
  pointLightPosition[2])); boundary_point[0] = pointLightPosition[0] +
  t*(vertices[i][0] - pointLightPosition[0]); boundary_point[1] =
  pointLightPosition[1] + t*(vertices[i][1] - pointLightPosition[1]);
          boundary_point[2] = boundary_z;
          boundary_points.push_back(boundary_point);
      }
  }*/

  // run gradient descent on the boundary points to find their optimal positions
  // such that they satisfy Fermat's principle
  /*if (!use_reflective_caustics && use_point_src) {
      for (int i=0; i<boundary_points.size(); i++) {
          for (int iteration=0; iteration<100000; iteration++) {
              double grad_x;
              double grad_y;
              gradient(pointLightPosition, boundary_points[i], vertices[i], 1.0,
  refractive_index, grad_x, grad_y);
  ...
  for (int i = 0; i < vertices.size(); i++) {
    ...
    std::vector<double> transmitted = {target_pts[i][0] - vertices[i][0],
...
Eigen::MatrixXd scaleAndTranslate(const Eigen::MatrixXd &mat, double newMin,
                                  double newMax) {
  ...
  // Apply the scaling formula: newValue = newMin + (oldValue - oldMin) *
  // (newMax - newMin) / (oldMax - oldMin)
  Eigen::MatrixXd scaled =
      (mat.array() - oldMin) * (newMax - newMin) / (oldMax - oldMin) + newMin;
...
int main(int argc, char **argv) {
  ...
  // Enable stdout suppression for JSON mode
  if (opts.json_progress) {
    original_cout_buffer = std::cout.rdbuf();
    std::cout.rdbuf(&null_buffer);
  }

  log_progress(opts.json_progress, "processing", "initialization", 0.05,
  ...
  if (!opts.uniform_src) {
    ...
    if (!load_input_density(opts.filename_src, density_src)) {
      std::cout << "Failed to load input \"" << opts.filename_src
                << "\" -> abort.";
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (!load_input_density(opts.filename_trg, density_trg)) {
    std::cout << "Failed to load input \"" << opts.filename_trg
              << "\" -> abort.";
    exit(EXIT_FAILURE);
  }
  ...
  if (opts.verbose_level >= 1) {
    ...
    std::cout << "Mesh dimensions: " << opts.mesh_width << " x " << mesh_height
              << std::endl;
  ...
  log_progress(opts.json_progress, "processing", "initialization", 0.10,
               "Initialized mesh and integrator");

  // export_triangles_to_svg(mesh.source_points, mesh.triangles, 1, 1,
  // opts.resolution, opts.resolution, "../triangles.svg", 0.5);
  // export_grid_to_svg(mesh.source_points, 1, 1, opts.resolution,
  // opts.resolution, "../grid.svg", 0.5);

  // scale the mesh such that there is a small margin around the boundary
  double margin = opts.mesh_width / res_x;
  scaleAndTranslatePoints(mesh.source_points, opts.mesh_width, mesh_height,
                          margin);

  // extract the x and y coordinates from the surface vertices
  for (int i = 0; i < mesh.source_points.size(); i++) {
    Eigen::Vector2d point = {mesh.source_points[i][0],
                             mesh.source_points[i][1]};
  ...
  if (!opts.uniform_src) {
    ...
    TransportMap tmap_src =
        runOptimalTransport(rotated_src, opts); // computes T(v->1)
    log_progress(opts.json_progress, "processing", "optimal_transport", 0.25,
                 "Solved source transport map");
    TransportMap tmap_trg =
        runOptimalTransport(rotated_trg, opts); // computes T(u->1)
    log_progress(opts.json_progress, "processing", "optimal_transport", 0.40,
                 "Solved target transport map");
  ...
  log_progress(opts.json_progress, "processing", "mesh_generation", 0.50,
               "Computing target points");

  // turn the moved 2d cordinates back into 3d points
  std::vector<std::vector<double>> trg_pts;
  for (int i = 0; i < mesh.source_points.size(); i++) {
    std::vector<double> point = {vertex_positions[i].x(),
                                 vertex_positions[i].y(), 0};
    trg_pts.push_back(point);
  }

  // trg_pts now contains a point cloud where each 3d point is the target point
  // of each light ray exiting the vertices of the mesh

  // export_grid_to_svg(trg_pts, 1, 0.5, opts.resolution, opts.resolution,
  // "../grid.svg", 0.5);
  ...
  // scaling will make the resulting image larger or smaller than the lens
  // itself
  // scalePoints(trg_pts, {8, 8, 0}, {0.5, 0.5, 0});

  // rotation is for example if you want to project an image on an angled
  // surface relative to the lens
  rotatePoints(trg_pts, {0, 0, 0});

  // here we translate the points to the focal plane. you can change the x and y
  // offsets aswel if you application disires it
  ...
  // we perform multiple outer iterations because when the caustic surface
  // changes shape, so do the disired normals of the surface
  for (int i = 0; i < 10; i++) {
    ...
    // given the final target points, we compute the vertex normal that steers
    // light towards the target points
    std::vector<std::vector<double>> normals =
        fresnelMapping(mesh.source_points, trg_pts, r);
    ...
    if (i == 0) {
      ...
      for (const auto &s : mesh.source_points) {
        min_sx = std::min(min_sx, s[0]);
        max_sx = std::max(max_sx, s[0]);
        min_sy = std::min(min_sy, s[1]);
        max_sy = std::max(max_sy, s[1]);
        min_sz = std::min(min_sz, s[2]);
        max_sz = std::max(max_sz, s[2]);
      }
      ...
      for (const auto &t : trg_pts) {
        min_tx = std::min(min_tx, t[0]);
        max_tx = std::max(max_tx, t[0]);
        min_ty = std::min(min_ty, t[1]);
        max_ty = std::max(max_ty, t[1]);
      }
      ...
      for (const auto &n : normals) {
        min_nx = std::min(min_nx, n[0]);
        max_nx = std::max(max_nx, n[0]);
        min_ny = std::min(min_ny, n[1]);
        max_ny = std::max(max_ny, n[1]);
        min_nz = std::min(min_nz, n[2]);
        max_nz = std::max(max_nz, n[2]);
      }
      ...
      for (size_t j = 0; j < trg_pts.size() && j < mesh.source_points.size();
           j++) {
        tgt_file << mesh.source_points[j][0] << " " << mesh.source_points[j][1]
                 << " " << trg_pts[j][0] << " " << trg_pts[j][1] << "\n";
      }
      tgt_file.close();
      std::cout << "DEBUG: Saved target points to tests/target_points.txt"
                << std::endl;
    }
  ...
  log_progress(opts.json_progress, "complete", "done", 1.0,
               "Mesh exported successfully");

  // Restore stdout for final message
  if (opts.json_progress && original_cout_buffer) {
    std::cout.rdbuf(original_cout_buffer);
  }

  if (!opts.json_progress) {
    std::cout << "\033[1;32m" << "Exported 3d model as " << final_output_path
              << " relative to this executable." << "\033[0m" << std::endl;
  }
}
