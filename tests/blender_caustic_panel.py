"""
Caustic Lens Renderer - Blender Panel Script
...
4. Click "Run Script"
5. In the 3D View sidebar (press N), find the "Caustic" tab
...
def register():
    # Unregister first if already registered (prevents duplicates)
    try:
        unregister()
    except Exception:
        pass
    ...
    bpy.types.Scene.caustic_props = bpy.props.PointerProperty(type=CausticProperties)
    
    # Print platform info on registration
    platform_info = get_platform_info()
    print(f"Caustic Lens Panel registered on {platform_info['system']}")
    if platform_info['is_wsl_available']:
